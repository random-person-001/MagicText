...
public class Player extends Mortal implements java.io.Serializable {
    ...
    private void looseCastDmgSpell(int damage, Item spell) {
        looseCastDmgSpell(damage, spell.range, spell.cost, spell.animation1, spell.animation2, spell.getAlting(), spell.getPathfinding(), spell.getName(), spell.getSplashRadius());
    }
    ...
    private void looseCastDmgSpell(int dmg, int rng, int cost, SpecialText anim1, SpecialText anim2, boolean alt, boolean tracking, String spellType, int splashRadius) {
        if (mana >= cost) {
            Spell toFire = new Spell(room, x, y, orientation, dmg, rng, anim1, anim2, alt, tracking, spellType);
